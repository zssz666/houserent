...
export default {
  ...
  methods: {
    ...
    async getOrderInfo(page = 1) {
      console.log('roles', this.$store.getters.getUserInfo.role);
    ...
    processData(res, house, user, page) {
      this.allList = res.data.map(item => {
        ...
        let imgUrl = '';
        try {
          // 检查img_url是否为字符串，若是则解析JSON
          if (typeof houseItem.img_url === 'string') {
            const imgArray = JSON.parse(houseItem.img_url);
            imgUrl = imgArray[0]; // 获取第一张图片
          } else if (Array.isArray(houseItem.img_url)) {
            imgUrl = houseItem.img_url[0]; // 如果已经是数组则直接获取
          }
        } catch (error) {
